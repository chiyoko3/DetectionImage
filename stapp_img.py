...
def main():
    ...
    # タイトルの表示
    st.title("アニメ「ちいかわ」登場人物3人（ちいかわ、はちわれ、うさぎ、を判別します")

    # アップローダの作成
    uploaded_file = st.file_uploader("画像を選んで貼り付けてね", type="jpg")
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],3) * 100  # 「ちいかわ」の確率(%)
        pred1 = round(prediction[1],3) * 100  # 「はちわれ」の確率(%)
        pred2 = round(prediction[2],3) * 100  # 「うさぎ」の確率(%)

        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"{pred0}％の確率で「ちいかわ」ですね！")
        elif classNo == 1:
            st.subheader(f"{pred1}％の確率で「はちわれ」ですね！")
        else:
            st.subheader(f"{pred2}％の確率で「うさぎ」ですね！")
